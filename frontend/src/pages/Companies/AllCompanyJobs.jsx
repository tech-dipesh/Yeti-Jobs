    ...
    <div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 p-8'>
        {data?.message && data?.message.map(({ uid, title, description, salary, job_type, experience_years, total_job_views, expired_at, company_name }) => (
          <Jobcomps key={uid} uid={uid} title={title} description={description} salary={salary} job_type={job_type} experience_years={experience_years} total_job_views={total_job_views} company_name={company_name} expired_at={expired_at}/>
        ))}
      </div>
    </div>
  )
