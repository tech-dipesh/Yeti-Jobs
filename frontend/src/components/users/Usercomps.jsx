    ...
    <div className='bg-slate-800/60 border border-slate-700 rounded-2xl p-6 flex flex-col gap-5'>
      <div className='flex items-center gap-2 lg:gap-4'>
        {profile_pic_url ? (
          ...
          <div className='h-14 w-24 rounded-2xl bg-slate-700  flex items-center justify-center text-xs font-bold'>
            Profile Picture Not Avaible
          </div>
        )}
        ...
        <div className=''>
          ...
          {skills ? (
            ...
            <p className='text-sm italic text-neutral-600 mt-1 text'>No skills listed</p>
          )}
        </div>
      </div>

      <span className={`text-xs font-semibold p-1 rounded-full w-fit text-white ${role === 'admin' ? 'bg-neutral-400 border border-neutral-500' :
